...
#### install mongodb (c'est long) (pour ubuntu demander a google)
```
brew install mongodb
```
#### on creer un nouveau rep pour mongo (celui de base est bloque a l'ecole)
```
cd && mkdir -p data/db
```
####run mongo 
...
#### Aller dans le dossier cloné (dans un autre term)
```bash
...
En s'inspirant de ` app/pages/Dashboard.jsx `

Creer : ``` app/pages/NewPage.jsx ``` et ``` app/containers/NewPage.jsx ```
...
Dans ` app/routes.jsx `, importer NewPage et ajouter cette ligne en dessous de la ligne du dashboard
```<Route path="newpage" component={NewPage} />```

Relancer le serveur,
acceder a localhost:3000/newpage
